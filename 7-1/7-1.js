...
const amount = categories.children;
console.log(amount);
console.log(`В списке ${amount.length} категории`);

let headline;
let count;

for (const item of amount) {
     headline = item.querySelector("h2").textContent;
     count = item.querySelector("ul").children.length ;
  console.log(`Категория: ${headline} `);
